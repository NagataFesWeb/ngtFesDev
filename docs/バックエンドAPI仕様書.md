...
#### `verify_line_token` (Edge Function)
* **概要**: LINE IDトークンを検証し、Supabase Auth Session を発行する。
* **Trigger**: Client (Next.js) -> Auth
* **引数**:
  * `id_token`: LINEから取得したIDトークン
* **処理**:
  1. LINE API でトークン検証
  2. `users` テーブルの UPSERT (`line_user_id` で検索)
  3. Supabase Custom Token (JWT) の生成と返却
  * **Note**: `role` は `users` テーブルの値に基づく。 `auth.users` と `users` テーブルのIDは一致させる。
...
#### `cast_vote` (Database Function)
* **概要**: カテゴリごとの投票（上書き or 新規）。直接テーブルINSERTは禁止。
* **権限**: Authenticated User (Guest)
* **引数**:
  * `p_project_id` (uuid)
  * `p_category` (text)
  * **Note**: 実行ユーザーIDは `auth.uid()` から取得する。
* **ロジック**:
  1. **整合性チェック**: `projects` テーブルを参照し、`project.type` が `p_category` と対応しているか確認 (例: food -> type='food')。不一致ならエラー。
  2. `ON CONFLICT (user_id, category) DO UPDATE SET project_id = ...` を実行
* **Response JSON (Success)**:
  ```json
  { "status": "ok" }
  ```
...
#### `operator_update_congestion` (Database Function)
* **概要**: レート制限付きで混雑度を更新する。セキュリティのため `SECURITY DEFINER` で実行。
* **権限**: Public (ただし内部でトークン検証を行う)
* **引数**: `p_operator_token`, `p_level`
* **処理**:
  1. `p_operator_token` を検証 → `class_id` 特定
  2. 対象プロジェクト特定
  3. 最終更新時刻をチェック (`now() - updated_at < 30 sec` ならエラー)
  4. `congestion` テーブル更新
* **Response JSON (Success)**:
  ```json
  { "status": "updated", "new_level": 3 }
  ```
...
* **権限**: Admin Only
* **引数**:
  * `p_target_table` (text): リセット対象 ('users', 'fastpass', 'votes', 'all')
...
#### `admin_get_vote_summary` (Database Function)
* **概要**: 投票結果の集計データを取得する（クライアント側でCSV変換して利用）。
* **権限**: Admin Only
* **引数**: なし
* **処理**:
  1. 全投票数、ユニークユーザー数などの統計を算出。
  2. プロジェクトごとの得票数を集計し、降順で取得。
* **Response JSON (Success)**:
  ```json
  {
    "meta": {
...
| `429` | `RATE_LIMIT_EXCEEDED` | 更新頻度制限 |
