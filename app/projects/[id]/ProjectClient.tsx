...
interface ProjectClientProps {
    project: Project
    initialCongestionLevel: number
    initialWaitTime: number
    initialSlots: FastPassSlot[]
    id: string
}
...
export function ProjectClient({ project, initialCongestionLevel, initialWaitTime, initialSlots, id }: ProjectClientProps) {
    const router = useRouter()
    const { session } = useSession()
    const { settings: systemSettings } = useSystemSettings()
    const { status: salesStatus, loading: salesLoading } = useFastpassSalesStatus()

    const [congestionLevel, setCongestionLevel] = useState<number>(initialCongestionLevel)
    const [waitTime, setWaitTime] = useState<number>(initialWaitTime)
    const [slots, setSlots] = useState<FastPassSlot[]>(initialSlots)
            ...
            .on(
                ...
                (payload) => {
                    if (payload.new) {
                        ...
                        // Note: waitTime is not easily real-time without another subscription or RPC call
                        // But congestion level is the primary real-time indicator here.
                    }
                }
            )
            .subscribe()
